...
const getGeo = async (city) => {
  const geoAllData = await axios.get(
    `${geoBaseURL}=${encodeURIComponent(city)}&maxRows=1&username=${
...
const getWeather = async (lat, lng, dayLength) => {
  if (dayLength >= 0 && dayLength <= 16) {
    const weatherAllData_F = await axios.get(
      `${weatherBaseURL_F}lat=${lat}&lon=${lng}&key=${weatherAPIKey}`
    );
    ...
    try {
      const weatherData_F = {
        ...
        description:
          weatherAllData_F.data.data[daysToGoIndex].weather.description,
    ...
    const weatherAllData_C = await axios.get(
      `${weatherBaseURL_C}lat=${lat}&lon=${lng}&key=${weatherAPIKey}`
    );
    try {
      const weatherData_C = {
...
const getImageC = async (country) => {
  const pixAllDataC = await axios.get(
    `${pixBaseURL}key=${pixAPIKey}&q=${encodeURIComponent(
      country
    )}&image_type=photo`
  );
...
const getImage = async (city) => {
  const pixAllData = await axios.get(
    `${pixBaseURL}key=${pixAPIKey}&q=${encodeURIComponent(
      city
    )}&image_type=photo`
  );
